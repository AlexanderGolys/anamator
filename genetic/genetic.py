...
class Approximant:
    ...
    def fitness(self, target_foo, norm='sup', sampling_rate=100):
        ...
        """
        if norm == 'sup':
            return 1/(max([abs(target_foo(x) - self.make_polynomial()(x)) for x in np.linspace(-1, 1, sampling_rate)])+1)
        if norm == 'int':
            return 1/(sum([abs(target_foo(x) - self.make_polynomial()(x)) for x in np.linspace(-1, 1, sampling_rate)])
                      / sampling_rate + 1)
...
def epoch_frame(target_func, approximators_list, generate_png=False, top_n=5):
    ...
    frame.add_axis_surface(x_bounds=(-1, 1), y_bounds=(-10.2, 10.2))
    frame.blit_axes(settings_axes, x_only=False)
...
if __name__ == '__main__':
    ...
    populations_ = sequential(target, population_size=150, deg=21, epochs=200,
                              selection_type='rank', default_std=4, save_king=True, p_c=.65, metric='int',
                              start=3, add_new=100)
    learning_curve(populations_, filename='symmetric_seq_big.png', inverse=True)
    populations_ = list(map(lambda x: x.census(), populations_))
    make_film(target, populations_, filename='symmetric_seq_big.mp4', fps=5, resolution=(1280, 720), step=1, top_n=5,
              number_of_frames=60, save_ram=True, id='_sym_seq_', read_only=False)
