...
def single_frame(partition, values, interval, interval_but_smol, radius, indices, *t, resolution=(1280, 720)):
    ...
    frame.axis_surface.blit_scale(objects.PredefinedSettings.t1b0white, x_interval=.05, x_length=.01)
    # frame.axis_surface.blit_scale(objects.PredefinedSettings.t2b0white, x_interval=.01, x_length=.015*t_0)
    ...
    list_of_for_zoom_centers = split_object_list([frame.axis_surface.transform_to_pixel_coordinates(ctr) for ctr in for_zoom_centers], 32, 0)
