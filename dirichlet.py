...
def single_frame_without_split(partition, values, interval, interval_but_smol, radius, indices, *t, resolution=(1280, 720)):
    ...
    frame = basic_func.OneAxisFrame(resolution, 'black', 100, 100)
    frame.add_axis_surface(x_bounds=basic_func.SingleAnimation.blend_lists([[interval[0]-.1, interval[1]+.1], interval_but_smol],t_0), y_bounds=(-.3, .8))
    frame.blit_axes(objects.PredefinedSettings.fhd_axis, x_only=False)
...
def animate_zoom(no_of_points, no_of_fixed, radius, interval, interval_but_smol, speed, resolution, filename='dirichlet_func.mp4',
...
if __name__ == '__main__':
    ...
    animate_zoom(88, 44, 6, (0, 1), (.4, .6), .1, (1920, 1080))
